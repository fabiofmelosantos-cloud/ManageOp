...
import { loadSpreadsheetSettings } from "@/lib/storage"

const ALLOWED_SHEETS = new Set(["honetop", "barritas"])

export async function GET() {
  try {
    const { url } = await loadSpreadsheetSettings()
    ...
    const summary = sheets.map((sheetName) => {
      ...
      return {
        sheet: sheetName,
        rows: rows.slice(0, 8).map((row) => Object.fromEntries(Object.entries(row).slice(0, 6))),
