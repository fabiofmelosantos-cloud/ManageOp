...
const initial = { bagsInRoom: "", bagKg: "0.5", lowerValueBags: "", totalToProduce: "", lot: "", expiry: "", requestedQuantity: "", palletQuantity: "", palletNumber: "", channel: "HQ" }
...
export default function HonetopProductionPage() {
  ...
  const bagWeight = Number(form.bagKg || 0)
  const requiredRawKg = unitsToProduce * bagWeight
  const requiredRawBags = Math.ceil(requiredRawKg / 25)
  const requiredMeBags = unitsToProduce
  const requiredMeScoops = unitsToProduce
  ...
  const producedRawKg = producedTotal * bagWeight
  const remainingRawKg = Math.max(requiredRawKg - producedRawKg, 0)
  const palletRawKg = Number(form.palletQuantity || 0) * bagWeight
  ...
  useEffect(() => { void loadMaterialRequests().then((all) => setRoomRequests(all.filter((request) => request.requester.toLocaleLowerCase().includes("honetop") && request.status === "in_production"))).catch(() => {})
  ...
  async function requestMaterials() {
    ...
    try {
      ...
      const requests: MaterialRequest[] = [
        { id: `request_honetop_me_bags_${Date.now()}`, stockItemId: "honetop-me-bags", materialName: "Sacos ME · Honetop", quantity: requiredMeBags, unit: "un", requester: "Produção Honetop", destinationRoom: "Honetop", status: "requested", requestedAt: now },
        { id: `request_honetop_me_scoops_${Date.now() + 1}`, stockItemId: "honetop-me-scoops", materialName: "Scoops ME · Honetop", quantity: requiredMeScoops, unit: "un", requester: "Produção Honetop", destinationRoom: "Honetop", status: "requested", requestedAt: now },
        { id: `request_honetop_raw_${Date.now() + 2}`, stockItemId: "honetop-raw-material", materialName: "Matéria-prima · Honetop", quantity: requiredRawKg, unit: "kg", requester: "Produção Honetop", destinationRoom: "Honetop", status: "requested", requestedAt: now },
      ]
  ...
  return <main className="min-h-screen bg-background"><div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-8"><header className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><Button asChild variant="ghost" size="icon"><Link href="/"><ArrowLeft /></Link></Button><div><p className="text-sm text-muted-foreground">Produção · Linha dedicada</p><h1 className="text-3xl font-bold tracking-tight">Honetop</h1></div></div><Button onClick={() => void closeProduction()}><CheckCircle2 className="mr-2 size-4" />Fechar produção</Button></header>{message && <p role="status" className="rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm">{message}</p>}
    ...
    <Card className="border-primary/30"><CardHeader><CardTitle>1. Quantidade a produzir e requisição</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="top-total">Quantidade a produzir</Label><Input id="top-total" type="number" min="1" value={form.totalToProduce} onChange={(event) => set("totalToProduce", event.target.value)} /></div><div><Label htmlFor="top-weight">Peso por unidade</Label><select id="top-weight" className="h-10 w-full rounded-md border bg-background px-3" value={form.bagKg} onChange={(event) => set("bagKg", event.target.value)}><option value="0.5">500 g</option><option value="1">1 kg</option></select></div></div><div className="grid gap-3 sm:grid-cols-4"><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">MP necessária</span><strong className="block text-lg">{requiredRawKg.toFixed(2)} kg</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Sacos de 25 kg</span><strong className="block text-lg">{requiredRawBags}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Sacos ME</span><strong className="block text-lg">{requiredMeBags}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Scoops ME</span><strong className="block text-lg">{requiredMeScoops}</strong></div></div><Button onClick={() => void requestMaterials()} disabled={requesting || unitsToProduce <= 0}>{requesting ? "A enviar..." : "Requisitar ao armazém"}</Button></CardContent></Card>
    <section className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]"><Card><CardHeader><CardTitle className="flex items-center gap-2"><Factory className="size-5" />Calculadora de produção</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-4 sm:grid-cols-2">{fields.map(([key, label, type]) => <div key={key}><Label htmlFor={key}>{label}</Label><Input id={key} type={type} min={type === "number" ? 0 : undefined} value={form[key]} onChange={(event) => set(key, event.target.value)} /> </div>)}<div><Label htmlFor="bagKg">Peso por unidade</Label><select id="bagKg" className="h-10 w-full rounded-md border bg-background px-3" value={form.bagKg} onChange={(event) => set("bagKg", event.target.value)}><option value="0.5">500 g</option><option value="1">1 kg</option></select></div></div><div className="grid gap-3 sm:grid-cols-4"><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">MP necessária</span><strong className="block text-lg">{requiredRawKg.toFixed(2)} kg</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Sacos de 25 kg</span><strong className="block text-lg">{requiredRawBags}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Sacos ME</span><strong className="block text-lg">{requiredMeBags}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Scoops ME</span><strong className="block text-lg">{requiredMeScoops}</strong></div></div><div className="flex justify-end"><Button onClick={() => void requestMaterials()} disabled={requesting || unitsToProduce <= 0}>{requesting ? "A enviar..." : "Requisitar ao armazém"}</Button></div><div className="grid gap-3 sm:grid-cols-4"><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">MP nesta palete</span><strong className="block text-lg">{palletRawKg.toFixed(2)} kg</strong><small>{palletRawBags} saco(s) de 25 kg</small></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Sacos produzidos</span><strong className="block text-lg">{producedTotal}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Falta produzir</span><strong className="block text-lg">{remainingToProduce}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">MP restante</span><strong className="block text-lg">{remainingRawKg.toFixed(2)} kg</strong></div></div><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Matéria-prima na palete</span><strong className="block text-lg">{palletRawKg.toFixed(2)} kg</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Sacos em sala</span><strong className="block text-lg">{bagsRemaining}</strong></div><div className="rounded-lg bg-muted p-3"><span className="text-xs text-muted-foreground">Falta produzir</span><strong className="block text-lg">{remainingToProduce}</strong></div></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => { const next = tubs + 1; setTubs(next); void save({ tubs: next }) }}><Plus className="mr-2 size-4" />Adicionar saco à cuba</Button><Button variant="ghost" onClick={() => { const next = Math.max(tubs - 1, 0); setTubs(next); void save({ tubs: next }) }}><X className="mr-2 size-4" />Retirar saco da cuba</Button></div></CardContent></Card>
