...
const initial = { productName: "", productionLot: "", totalToProduce: "", palletQuantity: "", palletNumber: "", channel: "HQ", wasteMp: "0", wasteMe: "0", bagsInRoom: "", bagKg: "", lowerValueBags: "", lot: "", expiry: "", requestedQuantity: "" }
...
export default function HonetopProductionPage() {
  ...
  const theoreticalRawKg = rawPerUnit * producedTotal
  const actualMpKg = mpPallets.reduce((sum, pallet) => sum + (pallet.bags + pallet.lowerValueBags) * 25, 0)
  ...
  const canClose = Boolean(form.productName.trim() && form.productionLot.trim() && unitsToProduce > 0 && pallets.length > 0 && producedTotal >= unitsToProduce)
  ...
  async function addPallet() {
    ...
    await fetch("/api/finished-products", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ product: form.productName || "Honetop", palletNumber: pallet.number, quantity: pallet.quantity, lot: form.productionLot || pallet.lot, expiryDate: pallet.expiry, productionDate: new Date().toISOString().slice(0, 10), channel: pallet.channel }) })
