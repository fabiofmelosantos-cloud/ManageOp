...
export function StockManagement() {
  ...
  const [form, setForm] = useState({ name: "", category: "raw_material" as StockCategory, unit: "kg", quantity: "", lot: "", expiryDate: "" })
  ...
  async function addItem() {
    ...
    const item: StockItem = { id: `stock_${Date.now()}`, name: form.name.trim(), category: form.category, unit: form.unit.trim() || "un", quantity, lot: form.lot.trim(), expiryDate: form.expiryDate, createdAt: new Date().toISOString() }
    await updateItems([...items, item]); setForm({ name: "", category: form.category, unit: form.unit, quantity: "", lot: "", expiryDate: "" }); toast.success("Material adicionado ao stock.")
  }
  ...
  async function changeStatus(request: MaterialRequest, status: MaterialRequestStatus) {
    const now = new Date().toISOString(); const updated = requests.map((entry) => entry.id === request.id ? { ...entry, status, ...(status === "approved" ? { approvedAt: now } : {}), ...(status === "transferred" ? { transferredAt: now } : {}) } : entry)
    await updateRequests(updated); toast.success(`Requisição ${statusLabels[status].toLowerCase()}.`)
  }
  ...
  return <Tabs defaultValue="stock" className="flex flex-col gap-6">
    ...
    <TabsContent value="stock" className="flex flex-col gap-6">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><PackagePlus /> Adicionar matéria-prima ou embalagem</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-2"><Label>Categoria</Label><Select value={form.category} onValueChange={(value) => setForm({ ...form, category: value as StockCategory })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="raw_material">Matéria-prima</SelectItem><SelectItem value="packaging">Material de embalagem</SelectItem></SelectContent></Select></div>
