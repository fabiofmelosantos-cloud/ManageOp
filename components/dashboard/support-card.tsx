...
type SupportTask = { id: string; product: string; bagMeasure: string; internalCode: string; quantityToLabel: number; quantityLabeled: number; wasteQuantity: number; operatorName: string; createdAt: string; completed?: boolean }
type FormState = { product: string; bagMeasure: string; internalCode: string; quantityToLabel: string }
type TaskUpdateState = { quantityLabeled: string; wasteQuantity: string; operatorId: string }
...
export function SupportCard({ fullPage = false }: { fullPage?: boolean }) {
  ...
  async function updateTask(task: SupportTask, update: TaskUpdateState) {
    ...
    const response = await fetch("/api/support-tasks", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: task.id, quantityLabeled: Number(update.quantityLabeled), wasteQuantity: Number(update.wasteQuantity), operatorId: operator.id, operatorName: operator.name }) })
    ...
    <div className={fullPage ? "mx-auto max-w-7xl" : ""}>
      <div className="flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-3" aria-labelledby="support-task-list-title">
          ...
          {tasks.length === 0 ? <p className="py-4 text-center text-sm text-muted-foreground">Nenhum registo de suporte.</p> : <div className="overflow-x-auto"><div className="min-w-[1050px] overflow-hidden rounded-lg border"><div className="grid grid-cols-[1.2fr_1.2fr_1fr_1fr_1fr_1.6fr_auto] gap-3 bg-muted/50 px-4 py-2 text-xs font-medium text-muted-foreground"><span>Produto / ME</span><span>Medida do saco</span><span>A colar</span><span>Já colada</span><span>Desperdício</span><span>Operador / execução</span><span>Estado</span></div>{tasks.map((task) => <article key={task.id} className={`grid grid-cols-[1.2fr_1.2fr_1fr_1fr_1fr_1.6fr_auto] items-center gap-3 border-t px-4 py-3 ${task.completed ? "bg-emerald-50 text-emerald-950 dark:bg-emerald-950/30 dark:text-emerald-100" : "bg-background"}`}><div><p className="font-semibold">{task.product}</p><p className="text-xs opacity-70">ME: {task.internalCode}</p></div><p className="text-sm">{task.bagMeasure}</p><p className="text-sm">{task.quantityToLabel}</p><p className="text-sm">{task.quantityLabeled || "—"}</p><p className="text-sm">{task.wasteQuantity || "—"}</p>{task.completed ? <p className="text-sm">{task.operatorName || "—"}</p> : <form className="flex items-center gap-2" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); void updateTask(task, { quantityLabeled: String(data.get("quantityLabeled") ?? ""), wasteQuantity: String(data.get("wasteQuantity") ?? ""), operatorId: String(data.get("operatorId") ?? "") }) }}><Input className="w-20" name="quantityLabeled" type="number" min="0" placeholder="Colada" required /><Input className="w-20" name="wasteQuantity" type="number" min="0" placeholder="Desperdício" required /><Select name="operatorId"><SelectTrigger className="w-36"><SelectValue placeholder="Operador" /></SelectTrigger><SelectContent>{workers.map((worker) => <SelectItem key={worker.id} value={worker.id}>{worker.name}</SelectItem>)}</SelectContent></Select><Button type="submit" size="sm">Guardar</Button></form>}<div className="flex items-center gap-2"><Button type="button" size="sm" variant={task.completed ? "secondary" : "outline"} onClick={() => void toggleCompleted(task)} className="gap-2"><Check data-icon="inline-start" />{task.completed ? "Concluída" : "Validar"}</Button><Badge variant={task.completed ? "default" : "secondary"} className={task.completed ? "bg-emerald-600 text-white hover:bg-emerald-600" : ""}>{task.completed ? "Concluída" : "Pendente"}</Badge></div></article>)}</div></div>}
        </section>
      </div>
    </div>
  </div>
  ...
  return <Link href="/support" className="block"><Card className="group aspect-square border-primary/20 transition-all hover:border-primary/50 hover:shadow-md active:scale-[0.98]"><CardContent className="flex h-full flex-col items-center justify-center gap-2 p-2 text-center sm:p-3"><LifeBuoy className="size-8 text-primary" aria-hidden="true" /><CardTitle className="text-xs sm:text-sm">Suporte</CardTitle></CardContent></Card></Link>
}
